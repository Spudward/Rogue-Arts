...
public class ExampleSceneController : MonoBehaviour
{
    ...
    // For opening the drawers
    public void ToggleDrawers()
    {
        foreach(GameObject Drawer in GameObject.FindGameObjectsWithTag("Drawer"))
        {
            Drawer.GetComponent<DrawerState>().hasChecked = false;
            Drawer.GetComponent<DrawerState>().ToggleDrawers();
        }
    }

    // For opening Cupboards
    public void ToggleCupBoard()
    {
        foreach(GameObject CupBoard in GameObject.FindGameObjectsWithTag("CupboardDoor"))
        {
            CupBoard.GetComponent<CupboardDoorState>().hasChecked = false;
            CupBoard.GetComponent<CupboardDoorState>().isMoving = true;
            CupBoard.GetComponent<CupboardDoorState>().ToggleCupBoardDoors();
        }
    }
